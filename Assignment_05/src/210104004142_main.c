...
void cleanup_resources(SharedBuffer *sharedBuffer){
	...
	pthread_cond_destroy(&sharedBuffer->notEmpty);		// Destroy the condition variable
	pthread_cond_destroy(&sharedBuffer->notFull);		// Destroy the condition variable
	pthread_barrier_destroy(&sharedBuffer->barrier);	// Destroy the barrier
...
void *worker(void *arg){
	...
	pthread_barrier_wait(&sharedBuffer->barrier);		// Barrier wait to ensure all workers reach this point before proceeding
	return NULL;
}
...
void add_task(SharedBuffer *sharedBuffer, const char *srcPath, const char *destPath){
	...
	pthread_cond_signal(&sharedBuffer->notEmpty);			// Signal that a new task is available
...
void handle_path(const char *src, const char *dest, SharedBuffer *sharedBuffer){
	if(is_directory(src)){						// If the source path is a directory
		...
		pthread_mutex_lock(&sharedBuffer->mutex);
		while(sharedBuffer->numTasks >= sharedBuffer->bufferSize){
			pthread_cond_wait(&sharedBuffer->notFull, &sharedBuffer->mutex);
		}
		add_task(sharedBuffer, src, destFile);
		pthread_cond_signal(&sharedBuffer->notEmpty);		// Signal that buffer is not empty now
		pthread_mutex_unlock(&sharedBuffer->mutex);
...
int main(int argc, char *argv[]){
	...
	pthread_mutex_lock(&sharedBuffer.mutex);
	sharedBuffer.done = 1;
	pthread_cond_broadcast(&sharedBuffer.notEmpty);
	pthread_mutex_unlock(&sharedBuffer.mutex);
	
	pthread_barrier_wait(&sharedBuffer.barrier);				// Main thread waits for all workers to reach the barrier
